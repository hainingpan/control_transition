...
def run_tensor(inputs):
    ...
    ct_q=CT_tensor(L=L,seed=seed,seed_vec=seed_C,seed_C=seed_C,x0=None,ancilla=ancilla,store_vec=False,store_op=True,store_prob=True,complex128=complex128,gpu=True,ensemble=ensemble,ensemble_m=ensemble_m,xj=xj)
    ...
    ct_r=CT_tensor(L=L,seed=seed,seed_vec=1+seed_C,seed_C=seed_C,x0=None,ancilla=False,store_vec=False,store_op=True,store_prob=True,complex128=complex128,gpu=True,ensemble=ensemble,ensemble_m=ensemble_m,xj=xj)
    ...
    gc.collect()
    torch.cuda.empty_cache()
    ct_r_=CT_tensor(L=L,seed=seed,seed_vec=1+seed_C,seed_C=seed_C,x0=None,ancilla=False,store_vec=False,store_op=True,store_prob=True,complex128=complex128,gpu=True,ensemble=ensemble,ensemble_m=ensemble_m,xj=xj)
    for _ in range(L**2*2):
...
if __name__=="__main__":
    ...
    with open('CT_En{:d}_Enm{:d}_pctrl({:.2f},{:.2f},{:.0f})_pproj({:.2f},{:.2f},{:.0f})_L({:d},{:d},{:d})_xj({:s})_seed{:d}_seedC{:d}{:s}{:s}_XEB.pickle'.format(args.es,args.es_m,*args.p_ctrl,*args.p_proj,*args.L,args.xj.replace('/','-'),args.seed,args.seed_C,'_128' if args.complex128 else '_64','_anc'*args.ancilla),'wb') as f:
