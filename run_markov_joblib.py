...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_jobs', '-n', type=int, default=-1, help='Number of parallel jobs (-1 for all available cores)')
    ...
    print(f'Total number of tasks: {len(inputs)}')
    print(f'Using {args.n_jobs} parallel jobs')
    
    st = time()
    
    # Run with joblib parallel processing
    results = Parallel(n_jobs=args.n_jobs)(
        delayed(run_markov)(input_data) for input_data in tqdm(inputs, desc="Processing")
    )
