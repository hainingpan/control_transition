...
def run_tensor(inputs):
    ...
    T_max=2*ct.L**2
    coherece_matrix_map = torch.zeros((T_max+1,L+1,L+1),dtype=torch.float64)
    ct_cpu=ct.vec.cpu()[...,0]
    coherece_matrix_map[0]=get_coherence(ct_cpu)

    for t_idx in range(T_max):
        ct.random_control(p_ctrl=p_ctrl,p_proj=p_proj)

        ct_cpu=ct.vec.cpu()[...,0]
        coherece_matrix_map[t_idx+1]=get_coherence(ct_cpu)
...
if __name__=="__main__":
    ...
    L0=args.L[0]
    rs=results.reshape((L_list.shape[0],p_ctrl_list.shape[0],p_proj_list.shape[0],2*L0**2+1,L0+1,L0+1))

    with open('CT_En{:d}_pctrl({:.2f},{:.2f},{:.0f})_pproj({:.2f},{:.2f},{:.0f})_L({:d},{:d},{:d})_xj({:s})_seed{:d}{:s}_coherence.pickle'.format(args.es,*args.p_ctrl,*args.p_proj,*args.L,args.xj.replace('/','-'),args.seed,'_128' if args.complex128 else '_64'),'wb') as f:
        pickle.dump(rs, f)
