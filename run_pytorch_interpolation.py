...
def convert_to_fraction(fraction_str):
    ...
    print(fractions)
    return frozenset(fractions)

def run_tensor(inputs):
    L,p_ctrl,p_proj,p_global,xj,complex128,seed,ancilla,ensemble,add_x,no_feedback=inputs
    ...
    for _ in range(T_max):
        ct.random_control(p_ctrl=p_ctrl,p_proj=p_proj,p_global=p_global)
...
if __name__=="__main__":
    ...
    parser.add_argument('--p_global','-p_global',type=float,nargs=3,default=[0,1,1],help='Parameters for p_global in the form [start, stop, num] to generate values with np.linspace (default: [0, 1, 11]).')
    ...
    p_global_list=np.linspace(args.p_global[0],args.p_global[1],int(args.p_global[2]))
    st=time.time()
    inputs=[(L,p_ctrl,p_proj,p_global,xj,args.complex128,args.seed,args.ancilla,args.es,args.add_x,args.no_feedback) for L in L_list for p_ctrl in p_ctrl_list for p_proj in p_proj_list for p_global in p_global_list]
    ...
    if not args.ancilla:
        rs=results.reshape((L_list.shape[0],p_ctrl_list.shape[0],p_proj_list.shape[0],p_global_list.shape[0],3,args.es))
        O_map,EE_map,TMI_map=rs[:,:,:,:,0,:],rs[:,:,:,:,1,:],rs[:,:,:,:,2,:]
        save_dict={"O":O_map,"EE":EE_map,"TMI":TMI_map,"args":args}
    else:
        rs=results.reshape((L_list.shape[0],p_ctrl_list.shape[0],p_proj_list.shape[0],p_global_list.shape[0],1,args.es))
        SA_map=rs[:,:,:,:,0,:]
        save_dict={"SA":SA_map,"args":args}


    with open('CT_En{:d}_pctrl({:.2f},{:.2f},{:.0f})_pproj({:.2f},{:.2f},{:.0f})_pglobal({:.2f},{:.2f},{:.0f})_L({:d},{:d},{:d})_xj({:s})_seed{:d}{:s}{:s}{:s}{:s}.pickle'.format(args.es,*args.p_ctrl,*args.p_proj,*args.p_global,*args.L,args.xj.replace('/','-'),args.seed,'_128' if args.complex128 else '_64','_anc'*args.ancilla,f'_x{args.add_x}'*(args.add_x!=0),'_nFB'*(args.no_feedback)),'wb') as f:
